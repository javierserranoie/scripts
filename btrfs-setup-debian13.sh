...
fstab_append_idempotent_or_die() {
    ...
    fstab_already_ok "${uuid}" "${mnt}" "${opts}" && {
        log "fstab entry already satisfies ${mnt}"
        return 0
    }
...
main() {
    ...
    while [[ "$#" -gt 0 ]]; do
        ...
        --layout)
            ...
            shift 2
            ;;
        --strict-empty-mountpoints)
            STRICT_MOUNTPOINT_LAYOUT=1
            shift
            ;;
        --relax-root-mount-verify)
            RELAX_ROOT_MOUNT_VERIFY=1
            shift
            ;;
        --accept-nonempty-mounts)
            ACCEPT_NONNULL_MOUNTS_CLI=1
            shift
            ;;
        --nocow-var-log)
            NOCOW_VARLOG_CLI=1
            ...
            ;;
        --skip-nocow-var-log)
            SKIP_NOCOW_VARLOG_LAYOUT=1
            shift
            ;;
        --skip-bootloader)
            SKIP_BOOTLOADER_STACK=1
            shift
            ;;
        --write-fstab)
            WRITE_FSTAB=1
